import type { ReactNode } from "react";
import navData from "../../data/home/Navigation.json";

const socialIcons: Record<string, ReactNode> = {
  Facebook: (
    <svg width="14" height="14" fill="currentColor" viewBox="0 0 24 24">
      <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" />
    </svg>
  ),
  LinkedIn: (
    <svg width="14" height="14" fill="currentColor" viewBox="0 0 24 24">
      <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z" />
    </svg>
  ),
  Instagram: (
    <svg width="14" height="14" fill="currentColor" viewBox="0 0 24 24">
      <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z" />
    </svg>
  )
};

const evIcon = (
  <svg width="16" height="16" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
    <path d="M12 22c5.523 0 10-4.477 10-10S17.523 2 12 2 2 6.477 2 12s4.477 10 10 10z" />
    <path d="M12 8v8" />
    <path d="M8 12h8" />
  </svg>
);

const dropdownIcons: Record<string, ReactNode> = {
  home: (
    <svg width="16" height="16" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
      <path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z" />
      <polyline points="9 22 9 12 15 12 15 22" />
    </svg>
  ),
  business: (
    <svg width="16" height="16" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
      <path d="M6 22V4a2 2 0 0 1 2-2h8a2 2 0 0 1 2 2v18Z" />
      <path d="M6 12H4a2 2 0 0 0-2 2v6a2 2 0 0 0 2 2h2" />
      <path d="M18 9h2a2 2 0 0 1 2 2v9a2 2 0 0 1-2 2h-2" />
      <path d="M10 6h4" />
      <path d="M10 10h4" />
      <path d="M10 14h4" />
      <path d="M10 18h4" />
    </svg>
  )
};

const Navigation = () => {
  if (!navData.enabled) return null;

  return (
    <nav className="nav sticky">
    <div className="nav-utility">
      <div className="container nav-utility-inner">
        <div className="nav-contact">
          <a href={navData.contact.phone.href}>
            <span className="icon">{navData.contact.phone.icon}</span> {navData.contact.phone.label}
          </a>
          <a href={navData.contact.email.href}>
            <span className="icon">{navData.contact.email.icon}</span> {navData.contact.email.label}
          </a>
        </div>
        <div className="nav-links-utility">
          {navData.utilityLinks.map((link, index) => (
            <span key={link.href}>
              <a href={link.href}>{link.label}</a>
              {index < navData.utilityLinks.length - 1 && (
                <span className="sep">|</span>
              )}
            </span>
          ))}
          <div className="nav-social">
            {navData.socialLinks.map((link) => (
              <a
                key={link.href}
                href={link.href}
                target="_blank"
                rel="noopener noreferrer"
                aria-label={link.label}
              >
                {socialIcons[link.label]}
              </a>
            ))}
          </div>
        </div>
      </div>
    </div>
    <div className="nav-main">
      <div className="container nav-main-inner">
        <a href="/" className="nav-logo">
          <img src={navData.logoSrc} alt={navData.logoAlt} />
        </a>
        <div className="nav-desktop">
          {navData.dropdowns.map((dropdown) => (
            <div key={dropdown.id} className="nav-dd-wrap" data-dropdown={dropdown.id}>
              <button
                type="button"
                className="nav-dd-btn"
                aria-expanded="false"
                aria-controls={`dd-${dropdown.id}`}
              >
                {dropdownIcons[dropdown.id]}
                {dropdown.label} <span className="chevron">{navData.chevron}</span>
              </button>
              <div id={`dd-${dropdown.id}`} className="nav-dd" hidden>
                {dropdown.links.map((link) => (
                  <a key={link.href} href={link.href}>
                    {link.label}
                  </a>
                ))}
              </div>
            </div>
          ))}
          {navData.primaryLinks.map((link) => (
            <a key={link.href} href={link.href} className="nav-link">
              {link.label === "EV Chargers" && evIcon}
              {link.label}
            </a>
          ))}
          <a href={navData.cta.href} className="btn btn-cta">
            {navData.cta.label}
          </a>
        </div>
        <button
          type="button"
          className="nav-mobile-toggle"
          aria-label={navData.mobile.openMenuLabel}
          data-mobile-toggle
        >
          <span className="hamburger"></span>
          <span className="hamburger"></span>
          <span className="hamburger"></span>
        </button>
      </div>
    </div>
    <div className="nav-mobile" data-mobile-menu hidden>
      <div className="nav-mobile-header">
        <a href="/" className="nav-mobile-logo">
          <img src={navData.logoSrc} alt={navData.logoAlt} />
        </a>
        <button type="button" aria-label={navData.mobile.closeMenuLabel} data-mobile-close>
          {navData.mobile.closeLabel}
        </button>
      </div>
      <div className="nav-mobile-links">
        {navData.dropdowns.map((dropdown) => (
          <div key={dropdown.id}>
            <button
              type="button"
              className="nav-mobile-accordion"
              data-accordion={dropdown.id}
            >
              <span>{dropdown.mobileLabel ?? dropdown.label}</span>
              <span className="arrow">{navData.mobileArrow}</span>
            </button>
            <div className="nav-mobile-panel" data-panel={dropdown.id} hidden>
              {dropdown.links.map((link) => (
                <a key={link.href} href={link.href}>
                  {link.label}
                </a>
              ))}
            </div>
          </div>
        ))}
        {navData.mobile.links.map((link) => (
          <a key={link.href} href={link.href}>
            {link.label}
          </a>
        ))}
        <a href={navData.mobile.cta.href} className="btn btn-cta mobile-cta">
          {navData.mobile.cta.label}
        </a>
      </div>
      <div className="nav-mobile-footer">
        <a href={navData.contact.phone.href}>
          {navData.contact.phone.icon} {navData.contact.phone.label}
        </a>
        <a href={navData.contact.email.href}>
          {navData.contact.email.icon} {navData.contact.email.label}
        </a>
      </div>
    </div>
    </nav>
  );
};

export default Navigation;
